...
int main(int argc, char *argv[])
{
    ...
    if (use_backend == TCP) {
        ///[Variant_TCP]~
        ///^
        ///fprintf(stderr, "TCP Backend Debloated, exiting.\n");
        ///return -1;
        ///^
        ...
        ///[Variant_TCP_PI]~
        ///^
        ///fprintf(stderr, "TCP PI Backend Debloated, exiting.\n");
        ///return -1;
        ///^
    ...
    for (i=0; i < UT_INPUT_REGISTERS_NB; i++) {
        ///[RTU_Write_Bits][TCP_Write_Bits][TCP_PI_Write_Bits]
        ASSERT_TRUE(tab_rp_registers[i] == UT_INPUT_REGISTERS_TAB[i], "FAILED (%0X != %0X)\n", tab_rp_registers[i], UT_INPUT_REGISTERS_TAB[i]);
    }
    ///~
